...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: transactions } = await supabase
      ...
      .select('id, type, amount, description, status, created_at, reference_id')
      .eq('user_id', userId)
    ...
    // For each chama, check if user has pending contributions
    ...
    // 7. Get user profile for reputation and verification status
    const { data: profileData } = await supabase
      .from('user_profiles')
      .select('reputation_score, verified')
      .eq('user_id', userId)
      .single();

    if (profileData) {
      dashboardData.reputationScore = profileData.reputation_score || 75;
